(() => {
  ...
  /***************************************************************************
   * Meowmoon Bowling v1.5.23 wombat-safe-placement-pilot
   * Adds a Wombat-only metadata-driven safe-placement pilot.
   * Design: no choices, no score, no frames, no losing, no ads, no timers.
   **************************************************************************/

  console.log("Meowmoon Bowling code loaded: v1.5.23 wombat-safe-placement-pilot");
...
function yellowchickhatchDrawAnimation(ctx, x, y, scale, elapsed, duration, variant = 0) {
  ...
  function startWombatPlacementTestMode() {
    ...
    if (!localHost || params.get("wombatPlacementTest") !== "1") return false;

    hideStartOverlay();
    game.level = 1;
    game.introDismissed = true;
    game.message = "Wombat placement test";
    enterPlayingPhase(game.message, true, "wombat-placement-test");

    const variant = clamp(Number(params.get("variant") || 0), 0, WOMBATBURROWBUILDER_PALETTES.length - 1);
    const progress = clamp(Number(params.get("progress") || 1), 0, 1);
    const position = params.get("position") || "center";
    const yMid = (layout.playTop + layout.playBottom) * 0.5;
    const xForPosition = position.includes("left") ? 0 : position.includes("right") ? view.w : view.w * 0.5;
    const yForPosition = position.includes("top") || position === "high"
      ? layout.playTop
      : position.includes("bottom") || position === "low"
        ? layout.playBottom
        : yMid;
    const startedAt = 1000;
    const pin = {
      id: "wombat-placement-test-pin",
      x: xForPosition,
      y: yForPosition,
      angle: 0,
      fallen: false,
      ...
      fading: false,
      removed: false,
      specialType: "wombatburrowbuilder",
      specialTriggered: true,
      rocket: {
        type: "wombatburrowbuilder",
        variant,
        startedAt,
        duration: WOMBATBURROWBUILDER_DURATION_MS,
        finishAt: startedAt + WOMBATBURROWBUILDER_DURATION_MS
      }
    };
    game.pins = [pin];
    game.ball = null;
    ...
    function drawWombatPlacementTestFrame() {
      const current = startedAt + progress * WOMBATBURROWBUILDER_DURATION_MS;
      render(current);
      const placement = wombatburrowbuilderLastPlacement;
      window.__meowmoonWombatPlacementTestSnapshot = {
        source: "actual-game-app-local-test-mode",
        ...
        view: { w: view.w, h: view.h, dpr: view.dpr },
        position,
        variant,
        progress,
        preferredAnchor: { x: pin.x, y: pin.y + layout.pinH * 0.58 },
        naturalScale: Math.min(view.w / 520, view.h / 520),
        placement
      };
      if (params.get("wombatPlacementDebug") === "1" && placement) {
        ctx.save();
        ctx.strokeStyle = "rgba(0, 75, 255, 0.65)";
        ctx.lineWidth = 2;
        ctx.strokeRect(
          placement.safeArea.left,
          placement.safeArea.top,
          placement.safeArea.right - placement.safeArea.left,
          placement.safeArea.bottom - placement.safeArea.top
        );
        ctx.strokeStyle = "rgba(255, 0, 120, 0.75)";
        ctx.strokeRect(
          placement.screenBounds.minX,
          placement.screenBounds.minY,
          placement.screenBounds.maxX - placement.screenBounds.minX,
          placement.screenBounds.maxY - placement.screenBounds.minY
        );
        ctx.restore();
      }
      requestAnimationFrame(drawWombatPlacementTestFrame);
    }
    requestAnimationFrame(drawWombatPlacementTestFrame);
    return true;
  }
  ...
  if (!startWombatPlacementTestMode()) {
    startLevel();
