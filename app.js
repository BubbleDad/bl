(() => {
  ...
  /***************************************************************************
   * Meowmoon Bowling v1.3.2
   * Fixes v1.3.1 gallery-helper crash in popcorn, Star Tree, and van-lift animations; keeps willow excluded.
  ...
  function updateReward(current) {
    if (game.phase === "resolving" && current >= game.resolvingUntil) {
      if (remainingUprightCount() === 0) {
        // Do not allow a lingering or failed special animation to block the next level.
        game.pins.forEach((pin) => { if (pin.rocket) pin.removed = true; });
        beginReward();
...
function drawJogStrollerSoloPin(pin, current) {
  ...
  drawJoggingStroller(pin.x + 2, pin.y + 20, 116, age, false);
}
